...
For example, Python can replace an existing destination from a file with
`destination.replace(Bag(path))`. In JavaScript, decoded JSON can be applied
with `destination.replace(Bag.fromJson(text))`.
