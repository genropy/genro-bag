...
"""Earthquake logger example (async) — active cache + subscriptions.

Same as earthquake_resolver_example.py but runs on asyncio event loop.
The timer subscription uses asyncio tasks natively — no while/sleep needed.

Usage:
    pip install genro-bag[contrib-resolvers]
    python earthquake_resolver_async_example.py
"""
