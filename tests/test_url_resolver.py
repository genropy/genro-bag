...
class TestUrlResolverQueryParams:
    """Dynamic query parameter tests."""

    def test_query_params_stored(self):
        """Query params are stored in resolver."""
        resolver = UrlResolver(
            'http://example.com/search',
            query_params=['status', 'limit']
        )
        assert resolver._kw['query_params'] == ['status', 'limit']

    def test_build_url_without_node(self):
        """URL without node returns base URL."""
        resolver = UrlResolver(
            'http://example.com/search',
            query_params=['status']
        )
        assert resolver.current_url == 'http://example.com/search'

    def test_build_url_with_node_attrs(self):
        """URL with node attributes includes query params."""
        bag = Bag()
        bag['search'] = UrlResolver(
            'http://example.com/search',
            query_params=['status', 'limit']
        )
        node = bag.get_node('search')
        node.set_attr(status='active', limit=10)
    ...
    def test_build_url_ignores_missing_params(self):
        """URL ignores None/missing params."""
        bag = Bag()
        bag['search'] = UrlResolver(
            'http://example.com/search',
            query_params=['status', 'limit']
        ...
        node = bag.get_node('search')
        node.set_attr(status='active')  # no limit
    ...
    def test_attr_change_resets_cache(self):
        """Changing a query param attribute resets resolver cache."""
        bag = Bag()
        bag['search'] = UrlResolver(
            'http://example.com/search',
            query_params=['status'],
        ...
        # Simulate that cache was updated
        from datetime import datetime
        resolver._cache_last_update = datetime.now()
        assert not resolver.expired

        # First access triggers subscription
        # (we can't actually call load() without network, but we can trigger subscribe)
        resolver._subscribe_to_attrs()

        # Now change the attribute - should reset cache
        node.set_attr(status='pending')

        # Cache should be reset (expired)
        assert resolver.expired

    def test_attr_change_non_query_param_no_reset(self):
        """Changing non-query-param attribute does NOT reset cache."""
        bag = Bag()
        bag['search'] = UrlResolver(
            'http://example.com/search',
            query_params=['status'],
        ...
        # Change a NON-query-param attribute
        node.set_attr(other_attr='value')
